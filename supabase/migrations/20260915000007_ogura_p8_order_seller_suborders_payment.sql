...
-- Migration: 20260915000007_ogura_p8_order_seller_suborders_payment.sql
-- Invariants:
-- 1. Order financial values derive 100% server-authoritatively from the P7 quote
-- 2. Multi-seller: Single parent order partitioned into seller_sub_orders
-- 3. Shipping fee is charged once at parent level, not duplicated on sub-orders
-- 4. Complete item snapshot on order_items protects historical evidence
-- 5. Atomic state transitions: payment captured -> order confirmed -> P6 inventory consumed
-- 6. Payment failure or expiry strictly prevents order confirmation & stock consumption
-- 7. Complete idempotency for repeated payment confirmations
-- ============================================================================

-- 1. IMMUTABILITY TRIGGERS FOR ORDERS, SUB-ORDERS & ORDER ITEMS
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
        -- Financial amounts, user_id, order_number and quote_id are immutable
...
-- 2. CREATE ORDER FROM AUTHORITATIVE QUOTE
...
BEGIN
    ...
    -- 2. Validate quote exists, belongs to customer, is unexpired and pending
    ...
    -- 3. Check for existing order on this quote (Idempotency)
    ...
    -- 4. Verify that valid P6 reservations exist for this quote
    ...
    -- Ensure NONE of the quote's reservations are expired or invalid
    IF EXISTS (
    ...
    -- 7. Create payment transaction record (pre-payment state: initiated)
    ...
    -- 9. Return order descriptor (seller sub-orders NOT yet created; created strictly upon payment capture)
    RETURN jsonb_build_object(
        'order_id', v_order_id,
...
-- 3. CONFIRM ORDER PAYMENT (PAYMENT CAPTURE -> P6 INVENTORY CONSUMPTION)
...
-- 5. CANCEL ORDER (RELEASES RESERVATIONS IF PLACED)
...
BEGIN
    ...
    -- In P8, cancellation is only supported on placed (unpaid) orders; confirmed orders require refund processing (P10)
    IF v_order.status != 'placed'::order_status THEN
...
-- 6. GET ORDER DETAILS (CUSTOMER & SELLER TENANCY)
...
BEGIN
    ...
    -- Determine caller perspective: Customer, Seller, or Admin
    ...
    ELSE
        -- Check if caller is seller with sub-orders on this order
        SELECT id INTO v_seller_id FROM public.sellers WHERE user_id = v_caller_id;

        IF v_seller_id IS NOT NULL THEN
            -- Seller viewing only their own sub-orders
            SELECT COALESCE(jsonb_agg(jsonb_build_object(
                'sub_order_id', sso.id,
                'sub_order_number', sso.sub_order_number,
                'seller_id', sso.seller_id,
