...
BEGIN
    ...
    -- Exclude sub-orders that have already been settled or fully refunded
    FOR v_sub IN
        SELECT sso.*
        FROM public.seller_sub_orders sso
        WHERE sso.seller_id = p_seller_id
          AND sso.status = 'delivered'
          AND sso.delivered_at IS NOT NULL
          AND (sso.delivered_at + INTERVAL '7 days') <= (p_period_end::timestamp + INTERVAL '1 day')
          AND (sso.delivered_at + INTERVAL '7 days') >= p_period_start::timestamp
          AND NOT EXISTS (
              SELECT 1 FROM public.financial_ledger_entries fle
              WHERE fle.sub_order_id = sso.id
                AND fle.payout_id IS NOT NULL
          )
    LOOP
        -- Check completed refunds on this sub-order
        ...
        IF v_sub_refunds >= v_sub.subtotal_paise THEN
            -- Fully refunded: seller receives no payout and owes nothing on this sub-order
            CONTINUE;
        END IF;
        ...
        v_refunds := v_refunds + v_sub_refunds;
    END LOOP;
