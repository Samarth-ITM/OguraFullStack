-- OGURA PHASE 6: INVENTORY RESERVATION ENGINE
-- Migration: 20260915000005_ogura_p6_inventory_reservation.sql

-- ============================================================================
-- 1. RLS POLICIES FOR INVENTORY RESERVATIONS
-- ============================================================================

-- Customers can view reservations linked to their own quotes; Admins can view all
...
-- ============================================================================
-- 2. READ-ONLY INVENTORY QUERY FUNCTIONS
-- ============================================================================

-- 2.1 Get genuinely purchasable available stock for a product variant
...
BEGIN
    ...
    IF v_is_mto IS TRUE THEN
        RETURN 999999; -- Virtual infinite stock for MTO
    END IF;
...
-- ============================================================================
-- 3. ATOMIC RESERVATION MUTATION FUNCTIONS
-- ============================================================================

-- 3.1 Atomically reserve inventory for a single variant under a checkout quote
...
BEGIN
    ...
    -- Ownership check: Caller must own the quote unless admin
    ...
    IF v_is_mto IS TRUE THEN
        -- MTO products do not consume finite physical stock
    ...
    -- Standard physical stock reservation with strict concurrency row-locking
    ...
    -- Invariant: available stock = quantity_on_hand - quantity_reserved
    ...
    -- Atomically increase quantity_reserved
    UPDATE public.inventory_items
    SET quantity_reserved = quantity_reserved + p_quantity,
        updated_at = CURRENT_TIMESTAMP
    WHERE id = v_inv_id;

    -- Insert active reservation record with authoritative 15-minute TTL bound to quote expiry
    INSERT INTO public.inventory_reservations (
        quote_id, variant_id, quantity, status, expires_at
    ) VALUES (
        p_quote_id, p_variant_id, p_quantity, 'held'::inventory_reservation_status, v_quote_expires_at
...
-- 3.2 Atomically reserve multiple variants for a quote in deterministic order
...
BEGIN
    ...
    -- Process items ordered by variant_id ASC to strictly eliminate database deadlocks
    FOR v_item IN
...
-- ============================================================================
-- 4. RESERVATION RELEASE FUNCTIONS
-- ============================================================================

-- 4.1 Release a single inventory reservation (idempotent)
...
BEGIN
    ...
    -- Idempotent check: if already released or expired, return true as safe no-op
    IF v_status = 'released'::inventory_reservation_status OR v_status = 'expired'::inventory_reservation_status THEN
        RETURN true;
    END IF;

    -- Terminal guard: committed reservations cannot be released
    ...
    -- If MTO, simply mark as released
    IF v_is_mto IS TRUE THEN
    ...
    -- Standard physical stock release
    SELECT quantity_on_hand, quantity_reserved
    INTO v_on_hand, v_reserved
    FROM public.inventory_items
...
-- 4.2 Release all active reservations for a quote (idempotent)
...
-- ============================================================================
-- 5. RESERVATION EXPIRY FUNCTIONS
-- ============================================================================

-- 5.1 Expire a single stale reservation (idempotent)
...
BEGIN
    ...
    -- Must be strictly past expiration timestamp
...
-- 5.2 Expire a batch of stale reservations using SKIP LOCKED to avoid contention
...
-- ============================================================================
-- 6. RESERVATION CONSUMPTION FUNCTIONS (USED BY P8 ORDER CONFIRMATION)
-- ============================================================================

-- 6.1 Atomically consume a reservation into permanently sold inventory
...
BEGIN
    ...
    -- Idempotent check: if already committed, return true (prevent double consumption)
    IF v_status = 'committed'::inventory_reservation_status THEN
        RETURN true;
    END IF;

    -- Inactive check: cannot consume released or expired reservations
    IF v_status = 'released'::inventory_reservation_status OR v_status = 'expired'::inventory_reservation_status THEN
    ...
    -- Expiry timestamp check under lock (prevents payment/expiry race conditions)
    IF v_expires_at < CURRENT_TIMESTAMP THEN
    ...
    -- If MTO, simply mark as committed
    IF v_is_mto IS TRUE THEN
        UPDATE public.inventory_reservations
    ...
    -- Physical stock deduction: decrement BOTH quantity_on_hand and quantity_reserved
    SELECT quantity_on_hand, quantity_reserved
    INTO v_on_hand, v_reserved
    FROM public.inventory_items
    WHERE variant_id = v_variant_id
...
-- 6.2 Atomically consume all active reservations for a quote (all-or-nothing)
...
DECLARE
    ...
    v_count INTEGER := 0;
BEGIN
    -- First verify that none of the quote reservations are expired
    IF EXISTS (
    ...
    -- Consume all held reservations in deterministic order
    FOR v_res IN
        SELECT id FROM public.inventory_reservations
        WHERE quote_id = p_quote_id
          AND status = 'held'::inventory_reservation_status
...
-- Allow authenticated users to view reservations and quotes according to RLS policies
