...
function AddressesPage() {
  ...
  const [backendAddrs, setBackendAddrs] = useState<Address[]>([]);
  const [local, setLocal] = useState<Address[]>([]);
  const [error, setError] = useState("");

  useEffect(() => {
    accountRepository.listAddresses?.().then((res) => {
      if (res && res.length > 0) {
        setBackendAddrs(res);
      }
    });
  ...
  const all = [...backendAddrs, ...list, ...local];
  ...
  const add = async () => {
    ...
    if (accountRepository.saveAddress) {
      const saved = await accountRepository.saveAddress(form);
      if (saved) {
        setBackendAddrs((prev) => [saved, ...prev]);
        setForm(BLANK);
        toast.success("Address saved to your OGURA account");
        return;
      }
    }
